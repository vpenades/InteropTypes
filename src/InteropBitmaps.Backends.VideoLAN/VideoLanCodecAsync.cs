...
namespace InteropBitmaps.Codecs
{
    ...
    public class VideoLanCodec
    {
        #region lifecycle

        public VideoLanCodec(Uri uri)
        {
            _SourceURI = uri;
        ...
        public IEnumerable<MemoryBitmap> Decode()
        {
            ...
            using (var mediaPlayer = new MediaPlayer(libvlc))
            {
                ...
                // Start recording
                if (!mediaPlayer.Play(media)) throw new InvalidOperationException();

                // Waits for the processing to stop
                try
                {
                    return ProcessThumbnailsAsync(processingCancellationTokenSource.Token);
                }
                catch (OperationCanceledException)
                { }

                return Enumerable.Empty<MemoryBitmap>();
            }
        }

        private IEnumerable<MemoryBitmap> ProcessThumbnailsAsync(CancellationToken token)
        {
